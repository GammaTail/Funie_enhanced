...
class GetTrainingPairs(Dataset):
    ...
    def get_file_paths(self, root, dataset_name):
        ...
        if dataset_name=='EUVP':
            ...
            input_path = os.path.join(root, dataset_name, 'input')
            target_path = os.path.join(root, dataset_name, 'target')

            filesA = sorted(glob.glob(os.path.join(input_path, "*.*")))
            filesB = sorted(glob.glob(os.path.join(target_path, "*.*")))

            print(f"Loaded {len(filesA)} input images and {len(filesB)} target images.")
...
class GetValImage(Dataset):
    ...
    def get_file_paths(self, root, dataset_name):
        ...
        if dataset_name == 'paired':
            ...
            print(f"Loaded {len(files)} input images.")
        else:
            raise ValueError(f"Unknown dataset_name: {dataset_name}")
